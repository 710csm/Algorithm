...
func solution(_ m:Int, _ n:Int, _ board:[String]) -> Int {
    ...
    while true {
        ...
        // 블록 제거하기
        let isEndGame: Bool = removeBlock(&result, &characterBoard, &visited)
...
func removeBlock(
    ...
    for i in 0..<characterBoard.count - 1 {
        for j in 0..<characterBoard[i].count - 1 {
            ...
            // 현재 블록이 지워지지 않았고, 주변 블록이 같으면 블록을 제거, 게임이 끝나지 않음
...
func moveBlock(_ characterBoard: inout [[Character]], _ visited: inout [[Bool]]) {
    for i in 0..<characterBoard.count - 1 {
        for j in 0..<characterBoard[i].count {
            // 방문했던 블록이면 제거하고 위에 있는 블록들의 방문 여부와 이미 지워진 블록인지 판단하여 블록들을 이동
            if visited[i][j] {
